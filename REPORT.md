...
Verification combines 15 unit tests, real browser execution across 20 integrated cases including one local-model assisted recovery, genuine discovery evidence from local MLX and browser WebGPU, generated-code execution, and separate responsive/color checks. Expected failures count as passing checks only when the verifier confirms the intended failure behavior. These tests establish the demonstrated workflow's behavior; they do not establish production readiness for banking systems.
